...
from utils.constants import color_palette, mp_categories_mapping
...
class Sem_Exp_Env_Agent(ObjectGoal_Env_hm3d):
    ...
    def plan_act_and_preprocess(self, planner_inputs):
        ...
        # plan
        if planner_inputs.get("done", False):
              self.last_action = None
              self.info["sensor_pose"] = [0., 0., 0.]
              obs, rew, done, info = super().step({'action': 0})  # 执行stop动作触发reset
              return np.zeros(self.obs.shape), self.fail_case, True, self.info
    ...
    def _plan(self, planner_inputs):
        ...
        if (stop and planner_inputs['found_goal'] == 1) or self.replan_count > 26:
            ...
            if exp_pred[eve_start_x, eve_start_y] == 0 and self.eve_angle > -60:
                ...
                action = 1  # Forward
            # 0: stop, 1: forward, 2: left, 3: right, 4: 抬头, 5: 低头
    ...
    def _visualize(self, inputs):
        ...
        # sem_map[m2] = 1
        # colored_mask[map_mask]=np.array([color_palette[1][2],color_palette[1][1],color_palette[1][0]])*255
        # sem_map[vis_mask] = 3
        # sem_map[edge_mask] = 3
        ...
        # # sem_map[goal_mask] = 4
        # sem_map_vis[goal_mask]=np.array(color_palette[12:15])*255
        ...
        if graph is not None :
            ...
            for edge in all_edges_with_data:
                # if len(list(graph.neighbors(edge[0]))) == 1 or len(list(graph.neighbors(edge[1]))) == 1 :
                #     continue
            ...
            for edge in relation:
                ...
                cv2.line(sem_map_vis, pt1, pt2, (255, 0, 0), 2)  # blue色线
        ...
        # sem_map_vis = cv2.resize(sem_map_vis[240:720,240:720], (480, 480),
        #                          interpolation=cv2.INTER_NEAREST)
        # sem_map_vis = sem_map_vis[240:720,240:720]
        sem_map_vis = cv2.resize(sem_map_vis, (480, 480),
                                 interpolation=cv2.INTER_NEAREST)
        # rgb_new = cv2.resize(self.rgb_vis,(240,320),interpolation=cv2.INTER_NEAREST)
        ...
        # pos = (
        #     (start_x * 100. / args.map_resolution - 240)
        #     ,
        #     (480 - start_y * 100. / args.map_resolution + 240)
        #     ,
        #     np.deg2rad(-start_o)
        # )
        ...
        if args.visualize:
            # Displaying the image (缩小到 60% 显示)
            display_scale = 0.6
            display_image = cv2.resize(self.vis_image, None, fx=display_scale, fy=display_scale)
            cv2.imshow("Thread {}".format(self.rank), display_image)
            cv2.waitKey(1)
