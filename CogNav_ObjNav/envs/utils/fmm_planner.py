...
class FMMPlanner():
    ...
    def set_multi_goal(self, goal_map,step):
        traversible_ma = ma.masked_values(self.traversible * 1, 0) #mask掉障碍物层(0),只剩空白区域
        traversible_ma[goal_map == 1] = 0 # 除去障碍物层的地图，目标为0，其他为1
        # cv2.imwrite("map/trav_"+str(step)+".png",traversible_ma*255)
        dd = skfmm.distance(traversible_ma, dx=1)  # 用于计算SDF
        dd = ma.filled(dd, np.max(dd) + 1) # 将mask的值替换为max(dd)+1, 即障碍物为最大距离
        self.fmm_dist = dd
        self.around[goal_map == 1] = 0
        # plt.imshow(dd, cmap='viridis', origin='upper', norm=colors.Normalize(vmin=np.min(dd), vmax=np.max(dd)))
        # plt.colorbar(label='Signed Distance')
        # plt.savefig("map/sdf_"+str(step)+".png",dpi=300,bbox_inches='tight')
        # plt.clf()
        return
