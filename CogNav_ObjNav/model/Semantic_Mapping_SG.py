...
class Semantic_Mapping_SG(nn.Module):
    def __init__(self, args):
        ...
        # self.cog_tokenizer,self.cog_model=load_model("/home/caoyihan/Code/THUDM/cogvlm2-llama3-chinese-chat-19B-int4")
        # detection xxx
    ...
    def updateSceneGraph(self,save_path,idx,exp_map):
        ...
        scene_graph = {}
        for relation,image in zip(relation_list,image_list) :
            rr = self.llm.get_relationship_prompt(relation[0],relation[1],image)
            if rr is not None or rr != "none":
                if relation[0] in scene_graph.keys():
                    scene_graph[relation[0]][relation[1]]=rr
                else :
                    scene_graph[relation[0]]={relation[1]:rr}
                if rr == "on":
                    rr_ins = "under"
                    ...
                    rr_ins = None
                if rr_ins is not None :
                    if relation[1] in scene_graph.keys():
                        scene_graph[relation[1]][relation[0]]=rr_ins
                    else :
                        scene_graph[relation[1]]={relation[0]:rr_ins}
        ########
        self.changed=[]
        ...
        # return self.objects,None,obstacle_map.cpu().numpy()
        # return self.objects,self.scene_graph,obstacle_map
        return self.objects,self.bg_objects,scene_graph,obstacle_map,wall
    def getGlobalMap(self,map_before,ori_obs,eve_angle,current_poses,idx,save_path):
        ...
        # current_map_stair[0,4]=torch.clamp(agent_height_stair_proj[:, :, :] / self.map_pred_threshold, min = 0.0, max = 1.0)
        # rot_mat_stair, trans_mat_stair = rot_tran_grid(st_pose, current_map_stair.size(), self.device)
        # rotated_stair = F.grid_sample(current_map_stair, rot_mat_stair, align_corners=True)
