...
from vl_prompt.p_manager import extract_integer_answer, extract_scores, extract_objects, \
    ...
    judge_Correspondence_prompt,judge_OneObject_prompt,discover_OneObject_prompt,get_room_prompt,query_node_prompt,query_node_prompt_txt,query_node_prompt_txt_o1,target_node_prompt,query_state_transition
        ...
        for object,olist in relation.items():
            ...
            if len(c) == len(olist):
                for i in range(len(olist)):
                    odict[olist[i]]=c[i]
                with open("relationship_0531_3/"+str(object)+".json", "w") as f:
